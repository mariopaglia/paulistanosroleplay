...
AddEventHandler('casino:taskStartSlots',function(index, data)
	...
	r_showNotification(source, 'insuficient_token não tem fichas suficientes!')
		...
		PerformHttpRequest(Config.slotsLoss, function(err, text, headers) end, 'POST', json.encode({
		embeds = {
			{ 
				title = "DIAMOND CASINO REGISTRO DE PERDA:",
				thumbnail = {
					url = "https://i.imgur.com/2beKQ7n.jpg"
				}, 
				fields = {
					{ 
					...
					{ 
						name = "**PERDA DO PLAYER:**",
						value = "**"..data.bet.." fichas**\n⠀"
					},

				}, 
				footer = { 
					text = os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S"),
					icon_url = "https://i.imgur.com/2beKQ7n.jpg"
				},
				color = 15906321 
			}
		}
			...
			if Config.Offset then
				...
				if rnd2 > Config.OffsetNum then w.b = w.b + 1.0 end
...
function CheckForWin(w, data)
	...
	if total >= 100000 then
					...
					updatePlayerChips(source)
		PerformHttpRequest(Config.slotsGain, function(err, text, headers) end, 'POST', json.encode({
		embeds = {
			{ 
