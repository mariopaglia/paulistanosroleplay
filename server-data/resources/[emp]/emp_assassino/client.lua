		...
		if not emservico then
			...
			if distance <= 30.0 then
				...
				if distance <= 1.2 then
					drawTxt("PRESSIONE  ~r~E~w~  PARA INICIAR A RETIRADA DOS ÓRGÃOS",4,0.5,0.93,0.50,255,255,255,180)
			...
			if distance <= 2100 then
				...
				repeat
					...
					if IsPedDeadOrDying(npc) and not IsPedAPlayer(npc) and distancia <= 1.5 and not IsPedInAnyVehicle(ped) and not IsPedInAnyVehicle(npc) and not selectnpc and GetPedType(npc) ~= 28 and not pedlist[npc] then
						drawTxt("PRESSIONE  ~r~E~w~  PARA RETIRAR OS ÓRGÃOS",4,0.5,0.93,0.50,255,255,255,180)
						if IsControlJustPressed(0,38) then
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
		if segundos > 0 then
			drawTxt("AGUARDE ~r~"..segundos.."~w~ SEGUNDOS ATÉ FINALIZAR A RETIRADA DOS ÓRGÃOS",4,0.5,0.93,0.50,255,255,255,180)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
