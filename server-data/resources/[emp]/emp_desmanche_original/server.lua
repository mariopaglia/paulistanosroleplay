...
function emP.checkVehicle()
	...
	if placa_user then
		...
		if #rows <= 0 then
			TriggerClientEvent("Notify",source,"importante","Veículo não encontrado na lista do proprietário.")
			return
		end
		if mName then
			if parseInt(rows[1].detido) == 1 then
				TriggerClientEvent("Notify",source,"importante","Veículo encontra-se apreendido na seguradora.")
				return
			end
			if mBanido then
				TriggerClientEvent("Notify",source,"importante","Veículos de serviço ou alugados não podem ser desmanchados.")
				return
			end
		end
		return true
