...
local servico = false
local selecionado = 0
local CoordenadaX = 953.194
local CoordenadaY = -973.39
		...
		if not servico then
			...
			if distance <= 30.0 then
				...
				if distance <= 1.2 then
					drawTxt("PRESSIONE  ~r~E~w~  PARA INICIAR ENTREGAS",4,0.5,0.93,0.50,255,255,255,180)
		...
		if servico then
			...
			if distance <= 30.0 then
				...
				if distance <= 2.5 then
					drawTxt("PRESSIONE  ~r~E~w~  PARA ENTREGAR FERRAMENTA",4,0.5,0.93,0.50,255,255,255,180)
					if IsControlJustPressed(0,38) and emP.checkPermission() then
