...
function emP.checkPayment(id, mod, health)
    ...
    if user_id then
		if buffado then
			...
			vRP.injectMoneyLimpo(user_id, parseInt(paylist[mod][id].pay * 1.2)) -- VALOR BUFFADO (+20%)
			TriggerClientEvent("vrp_sound:source", source, 'coins', 0.5)
			TriggerClientEvent("Notify", source, "sucesso", "Você recebeu <b>R$ " .. vRP.format(parseInt(paylist[mod][id].pay * 1.2)) .. "</b> (Emprego Buffado).")
		else
			vRP.antiflood(source, "caminhoneiro", 3)
			vRP.injectMoneyLimpo(user_id, parseInt(paylist[mod][id].pay)) -- VALOR NORMAL
			TriggerClientEvent("vrp_sound:source", source, 'coins', 0.5)
			TriggerClientEvent("Notify", source, "sucesso", "Você recebeu <b>R$ " .. vRP.format(parseInt(paylist[mod][id].pay)) .. "</b>.")
		end
    end
end
-----------------------------------------------------------------------------------------------------------------------------------------
