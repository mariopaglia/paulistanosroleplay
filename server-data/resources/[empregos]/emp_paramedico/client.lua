...
local CoordenadaX = 311.21 -- 311.21,-594.26,43.29
local CoordenadaY = -594.26
...
local locs = {
	...
	[29] = { ['x'] = 378.01, ['y'] = -597.85, ['z'] = 28.07, ['xp'] = -828.35, ['yp'] = -1217.91, ['zp'] = 6.36, ['h'] = 320.34 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- PEDLIST
-----------------------------------------------------------------------------------------------------------------------------------------
			...
			if distance <= 5.0 then
				...
				if distance <= 1.2 then
					...
					if IsControlJustPressed(0,38) and emP.checkPermission() then
						...
						TriggerEvent("Notify","sucesso","Você entrou iniciou a rota")
					end
				end
			end
		end
	...
	while true do
		...
		if emservico then
			...
			if distance <= 50.0 and IsVehicleModel(vehicle,GetHashKey("ambulance")) then
				DrawMarker(21,locs[selecionado].x,locs[selecionado].y,locs[selecionado].z+0.20,0,0,0,0,180.0,130.0,2.0,2.0,1.0,240,200,80,100,1,0,0,1)
						...
						if DoesEntityExist(passageiro) then
							while true do
								...
								if not IsPedSittingInVehicle(passageiro,vehicle) then
									DrawMarker(21,x2,y2,z2+1.3,0,0,0,0,180.0,130.0,0.6,0.8,0.5,240,200,80,100,1,0,0,1)
								end
