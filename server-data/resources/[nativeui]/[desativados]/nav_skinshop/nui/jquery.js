...
$(function() {
    ...
    document.onkeyup = function(data){
        if (data.which == 27 && !alreadyPressed) {
            alreadyPressed = true;
            $.post("http://nav_skinshop/exit");
        }
        if(data.which == 65){ //a
            $.post('http://nav_skinshop/rotate', JSON.stringify("left"));
        }
        if(data.which == 68) { //d
            $.post('http://nav_skinshop/rotate', JSON.stringify("right"));
        }
        if(data.which == 88) { //levantar
            $.post('http://nav_skinshop/rotate', JSON.stringify("handsup"));
        }
        if(data.which == 39) { //right
            //alert("Teste")
            $.post('http://nav_skinshop/color', JSON.stringify("right"));
        }
        if(data.which == 37) { //left
            //alert("Teste")
            $.post('http://nav_skinshop/color', JSON.stringify("left"));
        }
    }
...
function menu() {
  ...
  $(".botao").each(function(i, ojb){
    $(this).click(function(){
        ...
        $.post("http://nav_skinshop/update", JSON.stringify([dados, tipo]));
    })
})
}