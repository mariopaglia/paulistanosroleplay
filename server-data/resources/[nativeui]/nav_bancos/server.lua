	...
	if user_id then
		...
		if parseInt(valor) > 0 then
			...
			if parseInt(multas) <= 0 then
				TriggerClientEvent("Notify",source,"importante","Multas inexistentes.")
			else
				TriggerClientEvent("Notify",source,"importante","<b>$"..vRP.format(parseInt(multas)).." dólares</b> em multas a pagar.")
			end
		end
	end
end