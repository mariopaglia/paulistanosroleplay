...
Citizen.CreateThread(function()
	...
	while true do 
		...
		elseif not nui then
			idle = 5
			DrawMarker(27, bennys[1],bennys[2],bennys[3]-0.97 ,0,0,0,0,0,0,3.0,3.0,1.0,0, 127, 255,80,0,0,0,0)
...
RegisterNUICallback("pagar",function(data)
	if cart["total"] and func.checkPayment(cart["total"]) then
		...
		-- TriggerEvent("progress",1000,"aplicando modificações")
		-- Wait(1000)
	...
	if data then
		if data.type == "reparar" then
			...
			if func.repairVehicle(vehicle,damage) then
				...
				TriggerEvent("progress",5000,"reparando")
				Wait(5000)
