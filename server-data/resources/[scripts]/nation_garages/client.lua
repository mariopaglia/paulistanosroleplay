...
RegisterKeyMapping('veichle:lock_fenix', '[L] Trancar Veiculo', 'keyboard', 'L')

RegisterCommand('veichle:lock_fenix', function(source, args, rawCommand)
	if cooldown2 < 1 then
		cooldown2 = 3
    	func.vehicleLock()
	end
...
-- VEHICLECLIENTLOCK
-----------------------------------------------------------------------------------------------------------------------------------------
function fclient.vehicleClientLock(vehid, lock)
...
-- RegisterCommand('+tryLockVehicle',function(source)
-- 	if cooldown < 1 then
-- 		startCooldown(1)
-- 		print(132)
-- 		local vehicle = getNearestVehicle(5)
-- 		if vehicle then
...
-- 				print(22)
-- 			end
-- 			-- TriggerServerEvent("nation:tryLockNearestVehicle",plate)
-- 		end
-- 	end
-- end)
...
function fclient.toggleLock(vnet)
	...
	if IsEntityAVehicle(vehicle) then
		local locked = GetVehicleDoorsLockedForPlayer(vehicle) >= 2
		local vehmodel = func.getVehicleInfo(GetEntityModel(vehicle)).name
		local vehname = config.getVehicleModel(vehmodel)
		if locked then
			TriggerServerEvent("nation:tryLockVehicle",VehToNet(vehicle))
