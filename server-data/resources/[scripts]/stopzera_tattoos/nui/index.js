...
$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
            $.post("http://stopzera_tattoos/L1Tattoos_clearLastPreviewTattoo",JSON.stringify({}))
			$.post("http://stopzera_tattoos/L1Tattoos_closeUI");
		}
	};
});
...
const updateButtons = () => {
    ...
    $(".box").click(function() {
        if ($(this).hasClass("selected")) {
            ...
            $.post("http://stopzera_tattoos/L1Tattoos_previewTattoo",JSON.stringify({tattooIndex: $(this).data("index"), tattooType: $(".boxed-types.selected").data("toped")}))
        }
    });    
}
...
const updateBoxRefresh = () => {
    ...
    $.post("http://stopzera_tattoos/L1Tattoos_getTattoosFromType",JSON.stringify({tattooType: boxIndex}),(data) => {
        $("#container").html(`
            <div class="sa"><i class="fas fa-bars" id="openmenu"></i></div>
            <h1 style="font-size:60px;">STUDIO</h1>
...
const getTattooList = () => {
    $("#tattoos-types").empty();
    $(".tattoos").empty();
    $.post("http://stopzera_tattoos/L1Tattoos_getTattooTypeList",JSON.stringify({}),(data) => {
...
$(".clickb").click(function() {
    $.post("http://stopzera_tattoos/L1Tattoos_saveTattoo",JSON.stringify({tattooIndex: $(".box.selected").data("index"), tattooType: $(".boxed-types.selected").data("toped")}))
});

$(".rotate.mais").click(function() {
    $.post("http://stopzera_tattoos/L1Tattoos_rotatePed",JSON.stringify({ rotateMais: 10 }))
});

$(".rotate.menos").click(function() {
    $.post("http://stopzera_tattoos/L1Tattoos_rotatePed",JSON.stringify({ rotateMenos: 10 }))
});