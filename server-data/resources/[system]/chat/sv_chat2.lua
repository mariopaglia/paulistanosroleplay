...
RegisterCommand('frp', function(source, args, rawCommand)
	...
	if user_id ~= nil then
		local player = vRP.getUserSource(user_id)
		local identity = vRP.getUserIdentity(user_id)
		TriggerClientEvent('chatMessage', -1, "[@Fora do RP] ".. GetPlayerName(source) .." ("..user_id..") ", {25, 102, 25}, rawCommand:sub(7))
		CancelEvent()
		print("[@Fora do Rp] " ..GetPlayerName(player).. " - " ..user_id .. ':' .. rawCommand:sub(7))
	end
end)
