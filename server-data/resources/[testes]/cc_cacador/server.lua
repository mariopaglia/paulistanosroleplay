...
function emp.checkWeight(mochila)
	...
	if true then
		if user_id then
			...
			if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("corda")*numCouro+vRP.getItemWeight("vibradorroubado")*numCarne <= vRP.getInventoryMaxWeight(user_id) then
				return true
			else
				return false
			end
...
AddEventHandler('cc_cacador:abaterpronto',function(abaterpronto)
	...
	local player = vRP.getUserSource(user_id)
	if true then
	vRP.giveInventoryItem(user_id,"corda",numCarne)
	TriggerClientEvent("itensNotify",source,"sucesso","Coletou","corda",numCarne,vRP.format(vRP.getItemWeight("corda")*parseInt(numCarne)))

    vRP.giveInventoryItem(user_id,"vibradorroubado",numCouro)
	TriggerClientEvent("itensNotify",source,"sucesso","Coletou","vibradorroubado",numCouro,vRP.format(vRP.getItemWeight("vibradorroubado")*parseInt(numCouro)))
	end
end)
