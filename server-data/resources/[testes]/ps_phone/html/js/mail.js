...
$(document).on('click', '.mail', function(e) {
    ...
    PS.Phone.Functions.SetupMail(MailData);
...
$(document).on('click', '#accept-mail', function(e) {
    ...
    $.post('http://ps_phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e) {
    e.preventDefault();
    var MailData = $("#" + OpenedMail).data('MailData');
    $.post('http://ps_phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0 + "vh"
...
PS.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(PS.Phone.Data.PlayerData.charinfo.firstname + "." + PS.Phone.Data.PlayerData.charinfo.lastname + "@PSus.nl");
...
PS.Phone.Functions.SetupMail = function(MailData) {
...
$(document).on('click', '#new-advert-submit', function(e) {
    ...
    if (Advert !== "") {
        ...
        $.post('http://ps_phone/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
        PS.Phone.Notifications.Add("fas fa-ad", "Adverts", "You cannot place an empty AD!", "#ff8f1a", 2000);
    }
});

PS.Phone.Functions.RefreshAdverts = function(Adverts) {
    $("#advert-header-name").html("@" + PS.Phone.Data.PlayerData.charinfo.firstname + "" + PS.Phone.Data.PlayerData.charinfo.lastname + " | " + PS.Phone.Data.PlayerData.charinfo.phone);
