...
const updateVault = () => {
	$.post("http://vrp_homes/requestVault",JSON.stringify({}),(data) => {
		...
		$('#inventory').html(`
			...
			<div class="esquerda">
				${nameList2.map((item) => (`
					<div class="item" style="background-image: url('http://177.54.144.176:8080/imagens/${item.index}.png');" data-item-key="${item.key}">
			...
			<div class="direita">
				${nameList.map((item) => (`
					<div class="item2" style="background-image: url('http://177.54.144.176:8080/imagens/${item.index}.png');" data-item-key="${item.key}">
						<div id="peso">${(item.peso*item.amount).toFixed(2)}</div>
						<div id="quantity">${formatarNumero(item.amount)}x</div>
						<div id="itemname">${item.name}</div>
					</div>
