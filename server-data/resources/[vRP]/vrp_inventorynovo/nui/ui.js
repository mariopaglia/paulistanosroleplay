...
$(document).ready(function () {
  ...
  document.onkeyup = function (data) {
    if (data.which == 27) {
      $.post("http://vrp_inventorynovo/invClose", JSON.stringify({}), function (datab) {});
    }
  };
});
...
var updateMochila = () => {
  ...
  $.post("http://vrp_inventorynovo/requestMochila", JSON.stringify({}), data => {
    ...
    ${nameList
      .map(
        ...
        <div class="row">
          <div class = "image" style = "background-image: url(http://189.127.164.170:8080/imagens/${item.index}.png)" > </div>
        </div>
        <div class="row">
...
$(document).ready(function () {
  ...
  $(document).on('click', '.options .option', function () {
    ...
    if (event == 'send' || event == 'drop') {
      ...
      var amount = 1;
      $.post(
        "http://vrp_inventorynovo/useItem",
        JSON.stringify({
  ...
  $(document).on('click', '.amount-option button', function () {
    ...
    if (event == 'send') {
      ...
      $.post(
        "http://vrp_inventorynovo/" + paramUrl,
        JSON.stringify({
          item: $el.data("item-key"),
          amount
        })
