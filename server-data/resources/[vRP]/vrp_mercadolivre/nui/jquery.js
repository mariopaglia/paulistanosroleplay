$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://vrp_mercadolivre/mercadolivreClose");
		}
	};
});
...
const updatemercadolivreList = () => {
	$.post("http://vrp_mercadolivre/requestmercadolivreList",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.mercadolivrelist.sort((a,b) => (a.itemname > b.itemname) ? 1: -1);
		$('#inicio').html(`
...
const updateinvList = () => {
	$.post("http://vrp_mercadolivre/requestinvList",JSON.stringify({}),(data) => {
		let i = 0;
...
$(document).on("click",".comprar",function(){
	let $el = $('.model.active');
	if($el){
		$.post("http://vrp_mercadolivre/mercadolivreBuy",JSON.stringify({
...
$(document).on("click",".retirar",function(){
	let $el = $('.model.active');
	if($el){
		$.post("http://vrp_mercadolivre/mercadolivreExtract",JSON.stringify({
			id: $el.attr('data-id-key'),
			userid: $el.attr('data-userid-key'),
			qtd: $el.attr('data-qtd-key'),
			itemid: $el.attr('data-itemid-key')
...
$(document).on("click",".vender",function(){
	...
	if($el && price > 0 && amount > 0){
		$.post("http://vrp_mercadolivre/mercadolivreSell",JSON.stringify({
			itemid: $el.attr('data-itemid-key'),
			amount,
			price,
			anony: anony.checked
