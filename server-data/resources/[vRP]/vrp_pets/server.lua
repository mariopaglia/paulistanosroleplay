...
AddEventHandler('vrp_pets:alimentarbixin', function()
	local _source = source
	local player = vRP.getUserId(_source)
	vRP.tryGetInventoryItem(player, 'comidapet', 1)
end)
...
AddEventHandler("vrp_pets:abrimenucompra", function()
	...
	for k, v in pairs(pets) do
		menudata[v.name] = {function (choice)
			...
			local Certeza = vRP.request(_source, "Tem certeza que deseja <b>comprar</b> um <b>"..pets[k].name.."</b> por R$ " .. vRP.format(parseInt(v.price)) .. "?", 30)
			if Certeza then
			if vRP.tryFullPayment(player, v.price) then
				...
				TriggerClientEvent("Notify", _source, "sucesso", "Você comprou um <b>" .. pets[k].name .. "</b> por <b>R$ " .. vRP.format(parseInt(v.price)).."</b>")
			else 
		...
		end, "<b>Valor:</b> R$ "..vRP.format(parseInt(v.price))..""}
	end
...
AddEventHandler("vrp_pets:MenudoPet", function (status, come, isInVehicle)
	...
	if come == 1 then
		...
		end, TriggerClientEvent("Notify", _source, "aviso", "Alimentação do PET: " .. status .. "%")}
