...
RegisterCommand("cor",function(source,args)
	...
	local arma = GetSelectedPedWeapon(ped)
		if tinta >= 0 and emP.checkPermission() then
			...
			TriggerEvent("Notify", "negado", "Necessário <b>Nitro Boost ou um plano VIP (consulte especificações) </b> para utilizar <b>/cor</b>")
        end
end,false)
-----------------------------------------------------------------------------------------------------------------------------------------
-- /VTUNING
...
----------------------------------------------------------------------------------------------------------------------------------------------------
-- LIMITAR VELOCIDADE DOS VEICULOS
----------------------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread( function()
    while true do 
        Citizen.Wait( 0 )
