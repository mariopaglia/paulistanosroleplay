			...
			if timers[id] == 0 or not timers[id] then
				...
				if taskResult then
					...
					SetTimeout(10000,function()
						...
						local qntdinheiro = math.random(2000,4000) -- 2000,4000
					...
					TriggerClientEvent("Notify",source,"aviso","Você errou, corra, a polícia foi acionada!")
					TriggerClientEvent('cancelaranimacao',source,head,x,y,z)
					vRPclient.setStandBY(source,parseInt(60))
					for l,w in pairs(policia) do
						local player = vRP.getUserSource(parseInt(w))
						if player then
							async(function()
								TriggerClientEvent('blip:criar:registradora',player,x,y,z)
								vRPclient.playSound(player,"Oneshot_Final","MP_MISSION_COUNTDOWN_SOUNDSET")
								TriggerClientEvent('chatMessage',player,"190",{65,130,255},"Tentativa de roubo na ^1Caixa Registradora^0, dirija-se até o local e intercepte o assaltante.")
								SetTimeout(20000,function() TriggerClientEvent('blip:remover:registradora',player) end)
							end)
						end
					end
