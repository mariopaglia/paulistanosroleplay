...
$(document).ready(function () {
    document.onkeydown = function (data) {
        if (data.keyCode == 27) {
            ...
            $.post('http://ph-hud/fechar', JSON.stringify({
                id: false
            }))
        }
    }
    ...
    $("#leftHeading").click(function () {
        $.post('http://vrp_skinshop/leftHeading', JSON.stringify({
            value: 10
        }));
    })
    ...
    $("#rightHeading").click(function () {
        $.post('http://vrp_skinshop/rightHeading', JSON.stringify({
            value: 10
        }));
    })
    ...
    $("#payament").click(function () {
        ...
        $.post('http://vrp_skinshop/payament', JSON.stringify({
            price: $('#total').text()
        }));
        $('#total').html('0');
        change = {};
            ...
            for (var i = 0; i <= item.drawa; i++) {
                $("footer").append(`
                    <div class="item-clothe" data-id="${i}" onclick="select(this)">
                        <div class="img-clothe" style="background-image: url('http://177.54.158.241:8080/roupas/${item.category}/${item.sexo}/${item.prefix}(${i}).jpg')">  
                            <div class="overlay">
                                <span>${i}</span>
                            </div>
                        </div>
...
function selectPart(element) {
    ...
    $.post('http://vrp_skinshop/changePart', JSON.stringify({
        part: dataPart
    }))
}
...
function select(element) {
    ...
    $.post('http://vrp_skinshop/changeCustom', JSON.stringify({
        type: dataPart,
        id: roupaID
    }));
}
...
$(".fa-angle-right").click(function () {
    $.post('http://vrp_skinshop/changeColor', JSON.stringify({
        type: dataPart,
        id: roupaID,
        action: "mais"
    }));
